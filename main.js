...
(async () => {
   ...
   if (urlAfterNav.includes('/login') || urlAfterNav.includes('signin') || titleAfterNav.toLowerCase().includes('sign in') || titleAfterNav.toLowerCase().includes('login')) {
      ...
      const stUserSelectors = [
         ...
         if (!stFilledUser) {
                 console.log('  WARN: SecurTime username field not found - dumping inputs:');
                 try {
                           const inputs = await punchPage.$$eval('input', els =>
   ...
   if (!punchDone) {
         console.log('[DEBUG] Dumping clickable elements on punch page:');
         for (const frame of punchPage.frames()) {
                 try {
